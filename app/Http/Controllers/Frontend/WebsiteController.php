...
class WebsiteController extends Controller
{
    ...
    {
        try {
            $cities = DB::table('spn_cities')->select('id', 'name')->where('name', 'like', '%' . $request->search . '%')->where('country_id', '=', $request->id)->paginate(10);
...
//                    }
            ...
            $quizPass = true;
            if (Auth::check()) {
                $hasQuiz = QuizTest::where('course_id', $course->id)->where('user_id', Auth::user()->id)->count();
                $hasPassQuiz = QuizTest::where('course_id', $course->id)->where('user_id', Auth::user()->id)->where('pass', 1)->count();

                if ($hasQuiz != $hasPassQuiz) {
                    $quizPass = false;
                }
            }
        ...
        try {
            if (!hasTable('newsletter_settings')) {
                ...
                if ($newsletterSetting->home_service == "Local") {
                    ...
                    if (env('GET_RESPONSE_STATUS') == "true") {
                        $list = $newsletterSetting->home_list_id;
