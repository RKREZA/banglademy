...
class AddEmailTemplateForCourseInvitation extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        $template = EmailTemplate::where('act', 'Course_Invitation')->first();
        if (!$template) {
            $template = new EmailTemplate();
            $template->act = 'Course_Invitation';
        }

        $shortCode = '{"name":"Student Name","course_name":"Course Name","instructor_name":"Instructor Name","course_url":"Course URL","price":"Price","about":"About course"}';
        $subject = 'Course Invitation';
        $br = "<br/>";
        $body = "Hello {{name}}," . $br . " We have launched new course {{course_name}}. {{instructor_name}} is instructor of this course." . $br . "
                {{about}}." . $br . "
                Course price : {{price}}" . $br . "
                Course Enroll url: {{course_url}}." . $br .
            "{{footer}}";
        $template->name = $subject;
        $template->subj = $subject;
        $template->browser_message = "We have launched new course {{course_name}}. {{instructor_name}} is instructor of this course." . $br . "
                {{about}}." . $br . "
                Course price : {{price}}" . $br . "
                Course Enroll url: {{course_url}}.";
